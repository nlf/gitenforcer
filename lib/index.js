...
var gitenforcer = module.exports = function (config) {
    ...
    if (!config.token) throw new Error('Must specify oauth token');
    ...
    this.github.authenticate({ type: 'oauth', token: this.config.token });
    this.pollRepos();
...
gitenforcer.prototype.checkStatus = function (repo, id, res) {
    ...
    function runMiddleware(pull, comments) {
        async.forEachSeries(middleware, function (func, cb) {
...
gitenforcer.prototype.pollRepos = function () {
    ...
    function checkWatched(repo) {
        if (!repos.hasOwnProperty(repo.name)) repos[repo.name] = repo;
        self.github.repos.getHooks({ user: repo.owner, repo: repo.name }, function (err, data) {
            if (err || data.length === 0) return;
            data.forEach(function (detail) {
                if (detail.name === 'web' && detail.config.url.match(self.config.baseUrl)) {
                    repos[repo.name].id = detail.id;
                }
            });
        });
    }
    ...
    function checkRepos(err, data) {
        ...
        var repos = data.map(function (item) {
            return { name: item.name, owner: item.owner.login };
        });
    ...
    //self.github.repos.getAll({ sort: 'updated', per_page: 100 }, checkRepos);
    //if (self.config.organization) {
        self.github.repos.getFromOrg({ org: self.config.organization, type: 'all', per_page: 100, sort: 'updated' }, checkRepos);
    //} else {
